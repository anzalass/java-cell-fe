...
export default function LaporanBarangKeluarSparepartPage() {
  ...
  const fetchBarangKeluar = async (customParams = {}) => {
    ...
    const res = await api.get(`barang-keluar-sparepart?${params.toString()}`, {
  ...
  const exportToExcel = (exportData, filename) => {
    ...
    const worksheetData = exportData.map((item, index) => ({
      No: index + 1,
      "Nama Barang": item.namaBarang || item.nama || "-",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sparepart Keluar");
  ...
  const handleExport = async () => {
    try {
      ...
      const timeStr = now
        ...
        .replace(/:/g, "-");
      const filename = `sparepart-keluar-${dateStr}-${timeStr}.xlsx`;
    ...
    <div className="p-2 sm:p-6 w-full mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">
              Laporan Barang Keluar Sparepart
            </h1>
            <p className="text-sm text-gray-500">
              Rekap stok sparepart yang keluar
            </p>
          </div>
        </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <label className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:border-indigo-300 transition">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Seluruh data sparepart keluar tanpa filter (mungkin ribuan
                    data)
                  </p>
                </div>
              </label>
      ...
      <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="md:w-full w-[200%] text-sm">
            ...
            <tbody className="divide-y divide-gray-200">
              {tableData?.length === 0 ? (
                <tr>
                  ...
                  >
                    Tidak ada data sparepart keluar
                  </td>
                </tr>
              ) : (
