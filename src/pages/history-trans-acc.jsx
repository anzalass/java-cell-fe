// src/components/TableSectionAccecoris.jsx
import {
  Eye,
  Trash2,
  Calendar,
...
export default function TableSectionAccecoris({
  ...
  // === QUERY: Fetch Transaksi Aksesoris ===
  const {
    data: queryData,
    isLoading,
    isError,
    ...
    queryFn: async () => {
      ...
      const res = await api.get(`transaksi-acc?${params.toString()}`, {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) =>
      api.delete(`/transaksi-acc/${id}`, {
        headers: { Authorization: `Bearer ${user?.token}` },
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["transaksiAcc"] });
      Swal.fire({
        title: "Dihapus!",
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-lg">
            ...
            <div className="overflow-auto rounded border">
              <table className="lg:w-full w-[140%] text-sm">
