...
export default function ModalTransaksiAcc({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    const fetchMaster = async () => {
      try {
        const res = await api.get("acc-master", {
  ...
  // Fetch SEMUA member saat modal dibuka (sekali saja)
  useEffect(() => {
    if (!isOpen) return;
  ...
  const addItem = (aks) => {
    ...
    const existing = items.find((i) => i.idAksesoris === aks.id);
    if (existing) {
      setItems((prev) =>
        prev.map((i) =>
          i.idAksesoris === aks.id
  ...
  const addByBarcode = (code) => {
    const aks = masterAksesoris.find(
      (a) => a.barcode?.toLowerCase() === code.trim().toLowerCase()
    );
    if (!aks) {
      Swal.fire({
        title: "Barcode Tidak Ditemukan",
        text: "Silakan periksa kembali barcode yang Anda masukkan.",
        icon: "warning",
  ...
  const addByManualId = (id) => {
    ...
    if (!aks) {
      Swal.fire({
        ...
        text: "ID aksesoris yang dimasukkan tidak valid atau tidak tersedia.",
        icon: "error",
        confirmButtonText: "OK",
      });
      return;
  ...
  const addByNama = (nama) => {
    const aks = masterAksesoris.find(
      (a) => a.nama?.toLowerCase() === nama.trim().toLowerCase()
    );
    if (!aks) {
      Swal.fire({
  ...
  const handleSave = async () => {
    if (items.length === 0) {
      Swal.fire({
        ...
        text: "Tambahkan minimal satu aksesoris sebelum menyimpan transaksi.",
        icon: "warning",
        confirmButtonText: "OK",
      });
      return;
    ...
    try {
      ...
      await api.post("transaksi-acc", payload, {
        headers: { Authorization: `Bearer ${user.token}` },
      });
      ...
      await Swal.fire({
        ...
        text: "Transaksi aksesoris telah disimpan.",
        icon: "success",
        timer: 1500,
        showConfirmButton: false,
      });
      ...
      <div className="bg-white rounded-2xl w-full max-w-6xl max-h-[95vh] overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-indigo-700 text-white p-6 relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold">Transaksi Aksesoris</h2>
              <p className="text-indigo-100 text-sm mt-0.5">
                Kelola penjualan aksesoris HP
              </p>
            </div>
          </div>
        </div>
