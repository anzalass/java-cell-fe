...
export default function TableSectionAccToday({ title, data = [], onSuccess }) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await api.delete(`/transaksi-acc/${id}`, {
        headers: {
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 -top-[100px] z-50">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl animate-in slide-in-from-bottom duration-300">
            ...
            <div className="space-y-3 mb-5">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Penempatan:</span>
                <span className="text-gray-900">
                  {openDetail.penempatan || "—"}
                </span>
              </div>
              <div className="flex justify-between">
                  ...
                  {openDetail.items?.map((x) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right text-gray-900">
                        Rp{" "}
                        {typeof x.Aksesoris.hargaJual === "number"
                          ? x.Aksesoris.hargaJual.toLocaleString("id-ID")
                          : "0"}
                      </td>
                      <td className="px-4 py-3 text-right text-gray-900">
                        Rp{" "}
                        {Number(
                          x.Aksesoris.hargaJual * x.quantity
